...
$(document).ready(function() {
    ...
    //===================================================================================
    var theday = new Date();
    theday = new Date(2022, 11, 31, 11, 0, 0);
    ...
    $('#happy-modal-v, #happy-modal-u').on('hidden.bs.modal', function () {
        const type = $(this).find('.congratulation-type').val() ?? null;

        $('#thankyou-modal .thank-you-image').attr('src', `images/icons/${type}-thank-you.gif`)
        $('#thankyou-modal').modal('show');
    });

    $('.congratulate-submit').on('click', function () {
        const url = $(this).data('url');
        const data = {
            'name': $(this).closest('.congratulation-form').find('.congratulation-name').val() ?? null,
            'content': $(this).closest('.congratulation-form').find('.congratulation-content').val() ?? null,
            'type': $(this).closest('.congratulation-form').find('.congratulation-type').val() ?? null
        };
        ...
        $(this).closest('.congratulation-form').find('.congratulation-name').val('');
        $(this).closest('.congratulation-form').find('.congratulation-content').val('');
...
function checkEventTimeLine() {
    ...
    $('.timeline-step').each(function () {
        let eventTime = Date.parse($(this).data('timeline-date-time'));

        if (timeNow >= eventTime) {
            $(this).addClass('timeline-step-is-active');
            numOfEventActive ++;
        }
    });

    const nearestEventTime = Date.parse($('.timeline-step-is-active').last().data('timeline-date-time'));
    const nextEventTime = Date.parse($('.timeline-step').not('.timeline-step-is-active').first().data('timeline-date-time'));
    const nearestAndNextEventPercent = ((timeNow - nearestEventTime) * 100) / (nextEventTime - nearestEventTime);
    const progressPercent = (numOfEventActive - 1 ) * 25 + Math.round(nearestAndNextEventPercent * 25 / 100);

    $('.timeline-step-progress').css('width', `${progressPercent}%`);
}

function changeUIMemoriesSlider()
{
    if ($('.owl-item.active').length > 5) {
        $('.owl-item.active').eq(4).nextAll().removeClass('active');
    }
    ...
    const owl_item_length = $('.owl-item.active').length;

    if (owl_item_length == 1) {
        $('.owl-item.active').eq(0).addClass('middle');

        return;
    }

    const middle_item = Math.floor( owl_item_length/ 2);

    $('.owl-item.active').eq(middle_item - 1).addClass('middle_beside').addClass('prev_mid');;
